...
class Vectorizer:
    ...
    def __init__(self, text, word_tokens, pristine_input, pristine_output):
        ...
        print('Vocab size (STORE THIS VALUE FOR FUTURE USE):', self.vocab_size)
...
def train(model, x, y, x_val, y_val, batch_size, num_epochs, checkpoint_filepath):
    print('Training...')
...
if __name__ == "__main__":
    ...
    out_len = 50  # Length of the output phrase
    ...
    data_file = "C:/Users/XacHi/Desktop/e2 super mind/result.txt"  # Source text file
    ...
    parser.add_argument("-m", "--model", action="store", required=False, dest="model", help="Path to pre-made model")
    ...
    parser.add_argument("-s", "--seed", action="store", required=False, dest='seed', help="Seed for generation(any text)")
    parser.add_argument("-v", "--vocab_size", action="store", required=False, dest='vocab_size', help="Vocab value for reuse model without all train data")
    ...
    model_file = data_file.lower().replace('.txt', '.keras')

    if args.generate == False:
        # Make model
        model = make_model(batch_size, vectorizer.vocab_size)
        # Train model
        train(model, x, y, x_val, y_val, batch_size, num_epochs, model_file)
        # Save model to file
